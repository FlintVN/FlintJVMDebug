...
export class FlintClientDebugger {
    ...
    private startReadStatusTask() {
        const timeoutCallback = async () => {
            if(this.client.isConnected() === true) {
                ...
                if(resp?.responseCode === FlintDbgRespCode.DBG_RESP_OK) {
                    const status = resp.readU8();
    ...
    private async readConsole() {
        ...
        if(resp?.responseCode === FlintDbgRespCode.DBG_RESP_OK) {
            const text = resp.readAllAsString();
            if(text != null) {
    ...
    private sendCmd(cmd: FlintDbgCmd, data?: Buffer, timeout: number = FlintClientDebugger.TCP_TIMEOUT_DEFAULT): Promise<FlintDataResponse | undefined> {
        return this.tcpMutex.lock(() => new Promise((resolve) => {
            ...
            txData[0] = 0x00;
            txData[1] = cmd & 0x3F;
            txData[1] |= (length << 6) & 0xC0;
            txData[2] = (length >>> 2) & 0xFF;
            txData[3] = (length >>> 10) & 0xFF;
    ...
    public async startDebugSessionRequest(program: string, timeout: number = 5000): Promise<boolean> {
        const txBuff = Buffer.alloc(2 + program.length + 1);
        FlintClientDebugger.putConstUtf8ToBuffer(txBuff, program, 0);
        const resp = await this.sendCmd(FlintDbgCmd.DBG_CMD_START_DEBUG_SESSION, txBuff, timeout);
        return resp?.responseCode === FlintDbgRespCode.DBG_RESP_OK;
    }
    ...
    private async removeBreakPoints(lineInfo: FlintLineInfo): Promise<boolean> {
        ...
        let bufferSize = 4;
        bufferSize += 2 + className.length + 1;
        bufferSize += 2 + methodName.length + 1;
        bufferSize += 2 + descriptor.length + 1;
    ...
    public async readExceptionInfo(): Promise<FlintExceptionInfo | undefined> {
        ...
        if(resp?.responseCode === FlintDbgRespCode.DBG_RESP_OK) {
            const type = resp.readString();
            const msg = resp.readString();
            return new FlintExceptionInfo(type, msg);
        }
        else
            return undefined;
    }
    ...
    private async readStackFrame(frameId: number): Promise<FlintStackFrame | undefined> {
        ...
        if(resp?.responseCode === FlintDbgRespCode.DBG_RESP_OK) {
            const currentStack = resp.readU32();
            ...
            if(currentStackIndex !== frameId)
                return undefined;
            const pc = resp.readU32();
            const clsName = resp.readString();
            const name = resp.readString();
            const desc = resp.readString();
            const lineInfo = FlintLineInfo.getLineInfoFromPc(pc, clsName, name, desc);
    ...
    public async restartRequest(timeout: number = 5000): Promise<boolean> {
        const resp = await this.sendCmd(FlintDbgCmd.DBG_CMD_RESTART, undefined, timeout);
        return resp?.responseCode === FlintDbgRespCode.DBG_RESP_OK;
    }

    public async terminateRequest(timeout: number = 5000): Promise<boolean> {
        const resp = await this.sendCmd(FlintDbgCmd.DBG_CMD_TERMINATE, undefined, timeout);
        return resp?.responseCode === FlintDbgRespCode.DBG_RESP_OK;
    }

    private async stepRequest(stepCmd: number, stepCodeLength: number): Promise<boolean> {
        this.currentStackFrames = undefined;
        if(stepCmd != FlintDbgCmd.DBG_CMD_STEP_OUT) {
            ...
            const resp = await this.sendCmd(stepCmd);
            return resp?.responseCode === FlintDbgRespCode.DBG_RESP_OK;
        }
    }
    ...
    private static getSimpleNames(name: string): string[] {
        ...
        for(let i = 0; i < tmp.length; i++) {
            ...
            if(dimension > 0 && tmp[i].charAt(dimension) !== 'L') {
                switch(tmp[i].charAt(dimension)) {
                    ...
                    default:  simple = tmp[i].substring(dimension); break;
                }
            }
            else {
            ...
            if(dimension > 0)
                simple = simple.concat("[]".repeat(dimension));
            ret.push(simple);
        }
        return ret;
    }
    ...
    private static getComponentSize(arrayDescriptor: string): number {
        let index = 0;
        ...
        for(let i = 0; i < valueInfos.length; i++) {
            if(FlintClientDebugger.isPrimType(valueInfos[i].type)) {
                ...
                if(valueInfos[i].value === undefined)
                    ...
                    const arrayLength = valueInfos[i].size / FlintClientDebugger.getComponentSize(valueInfos[i].type);
    ...
    private async readLocalRequest(stackFrame: FlintStackFrame, variable: number | string): Promise<FlintVariableValue | undefined> {
        ...
        if(!(resp && resp.responseCode === FlintDbgRespCode.DBG_RESP_OK))
            return undefined;
        const size = resp.readU32();
        let value = (variableType === 2) ? resp.readU64() : resp.readU32();
    ...
    private async readFieldRequest(reference: number, fieldInfo: FlintFieldInfo): Promise<FlintVariableValue | undefined> {
        const txBuff = Buffer.alloc(4 + 2 + fieldInfo.name.length + 1);
        ...
        const size = resp.readU32();
        let value = isU64 ? resp.readU64() : resp.readU32();
    ...
    private async readArrayRequest(reference: number, index: number, length: number, arrayType: string): Promise<FlintVariableValue[] | undefined> {
        ...
        if(!(resp && resp.responseCode === FlintDbgRespCode.DBG_RESP_OK))
            return undefined;
        const compSize = FlintClientDebugger.getComponentSize(arrayType);
        const compType = arrayType.substring(1);
        const actualLength = resp.getDataLength() / compSize;
        const ret: FlintVariableValue[] = [];
        if(compSize === 1) {
            for(let i = 0; i < actualLength; i++) {
                const name = '[' + i + ']';
                const u8val = resp.readU8();
                const value = (u8val & 0x80) ? -(0x80 - (u8val & 0x7F)) : u8val;
                ret.push(new FlintVariableValue(name, compType, value, compSize));
            }
            return ret;
        }
        else if(compSize === 2) {
            for(let i = 0; i < actualLength; i++) {
                const name = '[' + i + ']';
                let value = resp.readU16();
                value = (compType === 'C') ? -(0x8000 - (value & 0x7FFF)) : value;
                ret.push(new FlintVariableValue(name, compType, value, compSize));
            }
            return ret;
        }
        else if(compSize === 4) {
            if(FlintClientDebugger.isPrimType(compType)) {
                for(let i = 0; i < actualLength; i++) {
                    const name = '[' + i + ']';
                    let value = resp.readU32();
                    ret.push(new FlintVariableValue(name, compType, value, compSize));
                }
                return ret;
            }
    ...
    private async readVariableRequest(reference: number): Promise<FlintVariableValue[] | undefined> {
        ...
        if(valueInfo.variable === undefined) {
            if(!FlintClientDebugger.isArrayType(valueInfo.type)) {
                ...
                const length = valueInfo.size / FlintClientDebugger.getComponentSize(valueInfo.type);
    ...
    private async readSizeAndTypeRequest(reference: number): Promise<[number, string] | undefined> {
        ...
        if(!(resp && resp.responseCode === FlintDbgRespCode.DBG_RESP_OK))
            return undefined;
        const size = resp.readU32();
        const typeName = resp.readString();
